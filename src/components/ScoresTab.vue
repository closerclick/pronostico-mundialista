...
import { teamAt, GROUP_PAIRS } from '../lib/standings'
...
function pairCodes (group: number, pair: number): string {
  const [a, b] = GROUP_PAIRS[pair]!
  return teamById(teamAt(group, a)).code + ' – ' + teamById(teamAt(group, b)).code
}
</script>
    ...
    <template v-else>
      ...
      <section v-if="d.positions.length" class="cat">
        ...
        <div v-for="(it, i) in d.positions" :key="'p' + i" class="item">
          <span class="what">{{ it.pos }}.º {{ GROUP_LETTERS[it.group] }} · {{ teamById(it.teamId).flag }} {{ teamById(it.teamId).code }}</span>
      ...
      <section v-if="d.bracket.length" class="cat">
        ...
        <div v-for="(it, i) in d.bracket" :key="'b' + i" class="item">
          <span class="what">{{ t(ROUND_KEY[it.roundKey] || 'bracket.r32Short') }} · {{ teamById(it.teamId).flag }} {{ teamById(it.teamId).code }}</span>
          <span class="pts">+{{ it.points }}</span>
        </div>
      </section>
      ...
      <section v-if="d.outcomes.length" class="cat">
        ...
        <div v-for="(it, i) in d.outcomes" :key="'o' + i" class="item">
          <span class="what">{{ GROUP_LETTERS[it.group] }} · {{ pairCodes(it.group, it.pair) }}</span>
          <span class="pts">+{{ it.points }}</span>
        </div>
      </section>
      ...
      <section v-if="d.exact.length" class="cat">
        ...
        <div v-for="(it, i) in d.exact" :key="'e' + i" class="item">
          <span class="what">{{ GROUP_LETTERS[it.group] }} · {{ pairCodes(it.group, it.pair) }} ({{ it.gh }}-{{ it.ga }})</span>
          <span class="pts">+{{ it.points }}</span>
        </div>
      </section>
    </template>
...
.item { display: flex; align-items: center; justify-content: space-between; padding: 0.3rem 0; border-top: 1px solid var(--line-soft); font-size: 0.86rem; }
.item:first-of-type { border-top: none; }
.what { min-width: 0; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
.pts {
