...
test('acuse de apertura: report envía sobre y start dispara notificación con el link', async ({ page }) => {
  ...
  const r = await page.evaluate(async () => {
    const mod = await import('/node_modules/@closerclick/closer-click-notifications/src/index.js')
    ...
    const url = 'https://mundial.closer.click/#BLOB123'
  ...
  expect(env.url).toBe('https://mundial.closer.click/#BLOB123')
  ...
  expect((r.notify!.opts.data as Record<string, unknown>).url).toBe('https://mundial.closer.click/#BLOB123')
